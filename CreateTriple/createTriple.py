...
def create_indexes(driver):
    index_queries = [
        "CREATE INDEX IF NOT EXISTS FOR (a:Artifact) ON (a.title)",
        ...
        "CREATE INDEX IF NOT EXISTS FOR (img:Image) ON (img.url)",
    ]
    with driver.session() as session:
...
def create_neo4j_triples(driver, artifacts):
    with driver.session() as session:
        ...
        for idx, artifact in enumerate(artifacts, 1):
            ...
            image_url = (artifact.get('image_url') or '').strip()
            ...
            session.run("""
                MERGE (a:Artifact {title: $title, object_id: $artifact_id})
                ON CREATE SET a.dimensions = $dimensions,
                              a.description = $description,
                              a.credit_line = $credit_line,
                              a.accession_number = $accession_number
                ON MATCH SET  a.dimensions = $dimensions,
                              a.description = $description,
                              a.credit_line = $credit_line,
                              a.accession_number = $accession_number
            """, {
            ...
            session.run("""
                ...
                MERGE (a:Artifact {title: $title, object_id: $artifact_id})-[:编号]->(oid)
            ...
            if period:
                ...
                for p in period_lines:
                    session.run("""
                        ...
                        MERGE (a:Artifact {title: $title, object_id: $artifact_id})-[:所属朝代]->(per)
                    """, {'period': p, 'title': title, 'artifact_id': artifact_id})
            ...
            if material:
                ...
                for mat in materials:
                    session.run("""
                        ...
                        MERGE (a:Artifact {title: $title, object_id: $artifact_id})-[:制作材质]->(mat_node)
                    """, {'material': mat, 'title': title, 'artifact_id': artifact_id})
            ...
            if type_val:
                ...
                for t in types:
                    session.run("""
                        ...
                        MERGE (a:Artifact {title: $title, object_id: $artifact_id})-[:文物品类]->(type_node)
                    """, {'type': t, 'title': title, 'artifact_id': artifact_id})
            ...
            if museum_name:
                session.run("""
                    ...
                    MERGE (a:Artifact {title: $title, object_id: $artifact_id})-[:收藏馆藏]->(m)
                """, {
                ...
                if museum_location:
                    session.run("""
                        ...
                        MERGE (m:Museum {name: $museum_name})-[:坐落地址]->(l)
                    """, {'location': museum_location, 'museum_name': museum_name})

            if image_url:
                session.run("""
                    MERGE (img:Image {url: $image_url})
                    MERGE (a:Artifact {title: $title, object_id: $artifact_id})-[:展示图片]->(img)
                """, {'image_url': image_url, 'title': title, 'artifact_id': artifact_id})
